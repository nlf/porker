...
class Porker extends EventEmitter {
    constructor({ connection, queue, errorThreshold = 1, retryDelay = '5 minutes', timeout = 15000, concurrency = 1, healthcheckPort = null } = {}) {
        ...
        this[Symbols.mutex] = Promise.resolve();
    ...
    async [Symbols.loop]() {
        ...
        await this[Symbols.mutex]; //Prevent the retry loop from clobbering our transaction
        this[Symbols.mutex] = new Promise(async (resolve, reject) => {
    ...
    async [Symbols.retryLoop]() {
        ...
        await this[Symbols.mutex]; //Prevent the main loop from clobbering our transaction
        this[Symbols.mutex] = new Promise(async (resolve, reject) => {
            while (!this[Symbols.stopped]) {
                await this[Symbols.pg].query('BEGIN');

...
class Porker extends EventEmitter {
    constructor({ connection, queue, errorThreshold = 1, retryDelay = '5 minutes', timeout = 15000, concurrency = 1, healthcheckPort = null } = {}) {
        ...
        Object.defineProperties(this, {
            [Symbols.client]: {
                value: new Pg.Pool(connection)
            },
            [Symbols.worker]: {
                value: new Pg.Pool(Object.assign({ max: 2 }, connection))
            },
            [Symbols.retryWorker]: {
                value: new Pg.Pool(Object.assign({ max: 2 }, connection))
            },
    ...
    async [Symbols.work]() {
        ...
        while (!this[Symbols.stopped]) {
            await client.query('BEGIN');
            const pendingJobs = await client.query(this[Symbols.queries].lockPendingJobs);
            ...
            if (futureJobs.length) {
                ...
                this[Symbols.workTimer] = setTimeout(() => {
                    ...
                    this[Symbols.work]();
                }, futureJobs[0].next_run - Date.now());
            }
            else if (recurringJobs.length) {
                clearTimeout(this[Symbols.workTimer]);
                this[Symbols.workTimer] = setTimeout(() => {

                    this[Symbols.work]();
                }, recurringJobs[0].future_run - Date.now());
            }

...
ALTER TABLE "porker_jobs" ADD COLUMN IF NOT EXISTS event text;
...
CREATE INDEX IF NOT EXISTS "porker_jobs_event_index" ON "porker_jobs" (event);`;
...
= `UPDATE "porker_jobs"
  ...
  WHERE id = ANY (
    SELECT id FROM "porker_jobs"
      WHERE
        (error_count = 0 AND event = ANY ($1::text[]) AND (started_at IS NULL OR (repeat_every IS NOT NULL AND COALESCE(started_at, TIMESTAMP '2000-01-01 00:00:00') + repeat_every <= NOW())))
      OR
        (error_count > 0 AND event = ANY ($2::text[]) AND error_count <= ${maxRetries})
      ORDER BY priority DESC, retry_at, (started_at + repeat_every), id
      LIMIT ${concurrency}
...
= `UPDATE "porker_jobs"
  ...
  WHERE id = ANY (
    SELECT id FROM "porker_jobs"
    WHERE event = ANY ($1::text[]) AND error_count = 0 AND (started_at IS NULL OR (repeat_every IS NOT NULL AND COALESCE(started_at, TIMESTAMP '2000-01-01 00:00:00') + repeat_every <= NOW()))
...
= `UPDATE "porker_jobs"
  ...
  WHERE id = ANY (
    SELECT id FROM "porker_jobs"
    WHERE event = ANY ($1::text[]) AND error_count > 0 AND error_count <= ${maxRetries}

...
UPDATE "porker_jobs"
  SET
    status = CASE
      WHEN repeat_every IS NOT NULL THEN
        'WAITING'
      ELSE
        'ERROR'
      END,
    start_after = CASE
      WHEN repeat_every IS NOT NULL THEN
        NOW() + repeat_every
      ELSE
        NOW() + COALESCE(retry_delay, '0'::interval)
      END,
    updated_at = NOW()
  WHERE id = ANY ($1::uuid[]);
`;
...
UPDATE "porker_jobs"
  ...
  WHERE porker_jobs.id = ANY (
    SELECT id FROM "porker_jobs"
      ...
      OR
        (status = 'ERROR' AND (SELECT count FROM error_counts WHERE job_id = id) <= max_retries AND event = ANY ($2::text[]) AND start_after <= NOW())
...
UPDATE "porker_jobs"
  ...
  WHERE porker_jobs.id = ANY (
    SELECT id FROM "porker_jobs"
    WHERE status = 'ERROR' AND event = ANY ($1::text[]) AND (SELECT count FROM error_counts WHERE job_id = id) <= max_retries
    ORDER BY priority DESC, start_after, created_at
    LIMIT $2::integer
    FOR UPDATE SKIP LOCKED
  )
...
SELECT id, start_after FROM "porker_jobs"
  WHERE start_after > NOW()
    AND status = 'ERROR'
    AND event = ANY ($1::text[])
    AND (SELECT count FROM error_counts WHERE job_id = id) <= max_retries
  ORDER BY priority DESC, start_after, created_at
  LIMIT 1;
`;
...
SELECT id, start_after FROM "porker_jobs"
  WHERE start_after > NOW()
    ...
    OR (status = 'ERROR' AND event = ANY ($2::text[]) AND (SELECT count FROM error_counts WHERE job_id = id) <= max_retries)
  ORDER BY priority DESC, start_after, created_at
  LIMIT 1;
`;
  ...
  const insertJobs = (jobs) => {
    ...
    const values = jobs.map(() => {
      return `($${++counter}::jsonb, $1, $2, $3::interval, $4::integer, $5::interval)`;
    });

    const query
= `
INSERT INTO "porker_jobs"
  (args, event, priority, repeat_every, max_retries, retry_delay) VALUES ${values.join(", ")}
  RETURNING id;
`;

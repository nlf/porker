...
= `UPDATE "porker_jobs"
  ...
  WHERE id = ANY (
    SELECT id FROM "porker_jobs"
      WHERE
        (error_count = 0 AND (started_at IS NULL OR (repeat_every IS NOT NULL AND COALESCE(started_at, TIMESTAMP '2000-01-01 00:00:00') + repeat_every <= NOW())))
      OR
        (error_count > 0 AND error_count <= ${maxRetries})
...
= `UPDATE "porker_jobs"
  ...
  WHERE id = ANY (
    SELECT id FROM "porker_jobs"
    WHERE error_count = 0 AND (started_at IS NULL OR (repeat_every IS NOT NULL AND COALESCE(started_at, TIMESTAMP '2000-01-01 00:00:00') + repeat_every <= NOW()))
...
= `UPDATE "porker_jobs"
  ...
  WHERE id = ANY (
    SELECT id FROM "porker_jobs"
    WHERE error_count > 0 AND error_count <= ${maxRetries}

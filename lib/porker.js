...
class Porker extends EventEmitter {
   ...
   */
  async publish(event, jobs, { priority = 0, repeat = null, maxRetries, retryDelay } = {}) {
    ...
    const res = await client.query(this.#queries.insertJobs(list), [event, priority, repeat, realMaxRetries, realRetryDelay, ...list]);

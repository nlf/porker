...
class Porker extends EventEmitter {
   ...
   */
  async publish(event, args, { priority = 0, repeat = null, maxRetries, retryDelay } = {}) {
    ...
    const res = await client.query(this.#queries.insertJob, [event, priority, repeat, realMaxRetries, realRetryDelay, args]);
  ...
  async unpublish(id) {
    const client = await this.#client.connect();
    await client.query(this.#queries.cancelJob, [id]);
    return client.release();
  }
  ...
  async #work() {
    ...
    while (!this.#stopped) {
      ...
      const { rows: currentJobs } = await client.query(currentJobQuery, [...payload, this.settings.concurrency]);
